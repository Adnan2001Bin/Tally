...
export async function updateGroupExpense(
  ...
  if (!existing) {
    throw new GroupError("Expense not found", 404, "EXPENSE_NOT_FOUND");
  }
  if (existing.created_by !== userId) {
    throw new GroupError("Only the person who added this expense can edit it", 403, "FORBIDDEN");
  }
  ...
  const expense = await prisma.$transaction(async (tx) => {
    await tx.personalExpense.deleteMany({
      where: { source_group_expense_id: expenseId },
    });
    ...
    for (const member of membersWithUsers) {
      ...
      await tx.personalExpense.create({
        data: {
          ...
          currency,
          source_group_expense_id: updated.id,
        },
      });
    }
